...
export default function SignupForm() {
  const [form, setForm] = useState<FormData>({
    name: "", company: "", building: "", role: "", contact: "", memo: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus("success");
      setForm({ name: "", company: "", building: "", role: "", contact: "", memo: "" });
    } catch (err) {
      setStatus("error");
  ...
  const inputClass = "w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-300 focus:outline-none focus:border-green-400 focus:ring-1 focus:ring-green-200 transition";
    ...
    <section id="signup" className="py-28 px-6 md:px-16 bg-white">
      <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-16 items-start">
        ...
        <div className="md:sticky md:top-28">
          <p className="text-green-600 text-xs font-semibold uppercase tracking-widest mb-5">사전 신청</p>
        ...
        <div>
          {status === "success" ? (
            <div className="border border-green-100 bg-green-50 rounded-2xl p-12 text-center">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-400 font-medium mb-1.5">이름 <span className="text-green-500">*</span></label>
                  <input type="text" name="name" value={form.name} onChange={handleChange} required placeholder="홍길동" className={inputClass} />
                </div>
                <div>
                  <label className="block text-xs text-gray-400 font-medium mb-1.5">직군 <span className="text-green-500">*</span></label>
              ...
              <div>
                <label className="block text-xs text-gray-400 font-medium mb-1.5">회사명 <span className="text-green-500">*</span></label>
              ...
              <div>
                <label className="block text-xs text-gray-400 font-medium mb-1.5">연락처 <span className="text-green-500">*</span></label>
              ...
              <div>
                ...
                <textarea name="memo" value={form.memo} onChange={handleChange} rows={3} placeholder="선호 요일, 직군 등 편하게 적어주세요" className={`${inputClass} resize-none`} />
              </div>
              {status === "error" && (
              ...
              <button
                ...
                className="w-full bg-green-600 hover:bg-green-700 disabled:bg-green-300 text-white font-semibold py-3.5 rounded-xl text-sm transition-colors"
              >
