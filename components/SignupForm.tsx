...
export default function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus("success");
      setForm({ name: "", company: "", building: "", role: "", contact: "", gender: "", birthYear: "", applyDate: "", menu: "", companion1: "", companion2: "", memo: "" });
    } catch (err) {
      setStatus("error");
  ...
  const menus = ["한식", "일식", "중식", "양식", "분식", "아무거나 OK"];
        ...
        <div>
          {status === "success" ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* 원하는 메뉴 */}
              <div>
                <label className="block text-xs text-gray-400 font-medium mb-1.5">원하는 메뉴</label>
                <select name="menu" value={form.menu} onChange={handleChange} className={inputClass}>
                  <option value="">선택 (없으면 무관)</option>
              ...
              {/* 동반 신청 */}
              <div>
                <label className="block text-xs text-gray-400 font-medium mb-1.5">동반 신청 <span className="text-gray-300">(최대 2인, 선택)</span></label>
                <div className="space-y-2">
                  <input type="text" name="companion1" value={form.companion1} onChange={handleChange} placeholder="동반자 1 — 이름 / 연락처" className={inputClass} />
                  <input type="text" name="companion2" value={form.companion2} onChange={handleChange} placeholder="동반자 2 — 이름 / 연락처" className={inputClass} />
                </div>
              </div>
